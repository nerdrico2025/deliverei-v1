...
function CustomTooltip({ active, payload }: TooltipProps<number, string>) {
  ...
  const formattedValue = formatCurrency(Number(data.total ?? 0));
...
export default function SalesChart({ data, loading = false, error = null }: SalesChartProps) {
  // Sort data by date (defensive against non-array)
  const sortedData = useMemo(() => {
    const src = Array.isArray(data) ? data : [];
    return [...src].sort((a, b) => new Date(a.data).getTime() - new Date(b.data).getTime());
  }, [data]);
  ...
  const stats = useMemo(() => {
    ...
    const values = src.map(d => Number(d.total ?? 0));
